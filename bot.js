...
client.on("messageCreate", (msg) => {
  ...
  if (message === "!commands") {
    ...
    const stream = discordTTS.getVoiceStream(message,{ lang: "en-PH" });
...
client.on("voiceStateUpdate", (before, after) => {
  ...
  if ((before.channelId && !after.channelId) || (before.channelId && after.channelId && before.channelId != after.channelId)) {
    ...
    if (before.channelId == IDs.voice3) {
      const stream = discordTTS.getVoiceStream(personTTS + " left",{lang: "en-PH"});
      const resource = createAudioResource(stream, { inputType: StreamType.Arbitrary, inlineVolume: true });
      playVoice(resource);
    }
  }
  ...
  if ((!before.channelId && after.channelId) || (before.channelId && after.channelId && before.channelId != after.channelId)) {
    ...
    if (after.channelId == IDs.voice3) {
      const stream = discordTTS.getVoiceStream(personTTS + " joined",{ lang: "en-PH" });
      const resource = createAudioResource(stream, { inputType: StreamType.Arbitrary, inlineVolume: true });
      playVoice(resource);
